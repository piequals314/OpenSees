...
int
TclModelBuilder_addTaperedForceBeamColumn(ClientData clientData,
                                          Tcl_Interp * interp,
                                          int inArgc,
                                          TCL_Char ** inArgv,
                                          Domain * theTclDomain,
                                          ...
                                          theTclBuilder)
{
    ...
    if (List == 0) {
        opserr <<
            "WARNING - TclModelBuilder_addTaperedForceBeamColumn - problem merging list\n";
        return TCL_ERROR;
    }
    ...
    if (Tcl_SplitList(interp, List, &argc, &argv) != TCL_OK) {
        opserr <<
            "WARNING - TclModelBuilder_addTaperedForceBeamColumn - problem spliting list\n";
        return TCL_ERROR;
    }
    Tcl_Free((char *) List);
    ...
    if ((strcmp(argv[6], "Lobatto") != 0) &&
        ...
        int nIP, secTag;
        double GJ, HP;             // additions made for Tapered Study
	...
	#ifdef DEBUG
	    fprintf(stdout, "The value of nIP in TclTaperedForceBeamColumnCommand is %i\n", nIP);
	#endif
        ...
        //HParr = new double[nIP];
        //GJarr = new double[nIP];
	    ...
	    printf("Section Tag = %d\n", secTag);
            ...
            // if "-sections" is found then there will be multiple section tags
            ...
            for (int i = 0; i < nIP; i++) {
                ...
                SectionForceDeformation *theSection =
                    theTclBuilder->getSection(secTag);
                if (theSection == 0) {
                    opserr << "WARNING section not found\n";
                    opserr << "Section: " << secTag;
                    opserr << argv[1] << " element: " << eleTag <<
...
#ifdef DEBUG
	printf("Before checking the iteration\n");
#endif
        while (argi < argc) {
            if (strcmp(argv[argi], "-iter") == 0) {
                if (argc < argi + 3) {
                    opserr <<
                        "WARNING not enough -iter args need -iter numIter? tol?\n";
                    opserr << argv[1] << " element: " << eleTag <<
                        endln;
                    return TCL_ERROR;
                }
                if (Tcl_GetInt(interp, argv[argi + 1], &numIter) !=
                    TCL_OK) {
                ...
                if (Tcl_GetDouble(interp, argv[argi + 2], &tol) !=
                    TCL_OK) {
                ...
                if (argc < argi + 2) {
                    opserr <<
                        "WARNING not enough -iter args need -mass mass??\n";
                    opserr << argv[1] << " element: " << eleTag <<
                        endln;
                    return TCL_ERROR;
                }
                if (Tcl_GetDouble(interp, argv[argi + 1], &mass) !=
                    TCL_OK) {
                    opserr << "WARNING invalid numIter\n";
                    opserr << argv[1] << " element: " << eleTag <<
                        endln;
	...
	#ifdef DEBUG
	    printf("Selecting the integration method\n");
	#endif
        if (beamIntegr == 0)
            beamIntegr = new LobattoBeamIntegration();

        if (ndm == 3) {
	    // Modified for Tapered Study 
	    if (strcmp(argv[1], "TapereddispBeamColumnS") == 0) {
		theElement = 
		    new TaperedDispBeamColumnSmoothing3d(eleTag, iNode, jNode, nIP, sections, *beamIntegr, 
						       	 *theTransf3d, mass);
	    }
	    else {
                theElement =
                    new TaperedDispBeamColumn3d(eleTag, iNode, jNode, nIP, sections, *beamIntegr,
                                                *theTransf3d, mass);
		#ifdef DEBUG
		    printf("Post creation of TaperedDispBeamColumn3d\n");
		#endif
            }
        }
    ...
    if (strcmp(argv[6], "Lobatto") == 0
        ...
        if (argc < 9) {
            ...
            printCommand(argc, argv);
            opserr << "Want: element " << argv[1] <<
                " eleTag? iNode? jNode? transfTag? UserDefined nIP? secTag1? ... pt1? ... wt1? ...\n";
            return TCL_ERROR;
        }
        ...
        for (i = 0, j = 8; i < numSections; i++, j++) {
            ...
            if (Tcl_GetDouble(interp, argv[j + numSections], &pt) !=
                TCL_OK) {
            ...
            if (Tcl_GetDouble(interp, argv[j + 2 * numSections], &wt)
                != TCL_OK) {
        ...
        for (i = 0; i < numSections; i++) {
            SectionForceDeformation *theSection =
                theTclBuilder->getSection(secs(i));
            if (theSection == 0) {
                opserr << "WARNING section not found\n";
                opserr << "Section: " << secs(i);
                opserr << "\n" << argv[1] << " element: " << eleTag <<
                    endln;
                return TCL_ERROR;
            }
            sections[i] = theSection;
        ...
        beamIntegr =
            new UserDefinedBeamIntegration(numSections, pts, wts);
    }
        ...
        if (argc < 12) {
            ...
            printCommand(argc, argv);
            opserr << "Want: element " << argv[1] <<
                " eleTag? iNode? jNode? transfTag? type secTagI? lpI? secTagJ? lpJ? secTagE?\n";
            return TCL_ERROR;
        }
        ...
        SectionForceDeformation *sectionI =
            theTclBuilder->getSection(secTagI);
        if (sectionI == 0) {
            opserr << "WARNING section not found\n";
        ...
        SectionForceDeformation *sectionJ =
            theTclBuilder->getSection(secTagJ);
        if (sectionJ == 0) {
            opserr << "WARNING section not found\n";
        ...
        SectionForceDeformation *sectionE =
            theTclBuilder->getSection(secTagE);
        if (sectionJ == 0) {
            opserr << "WARNING section not found\n";
        ...
        if (argc < 9) {
            ...
            printCommand(argc, argv);
            opserr << "Want: element " << argv[1] <<
                " eleTag? iNode? jNode? transfTag? UserHinge secTagE? npL? secTagL1? ... ptL1? ... wtL1? ... npR? secTagR1? ... ptR1? ... wtR1? ...\n";
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetInt(interp, argv[argStart + 3 * npL + 1], &npR) !=
            TCL_OK) {
        ...
        for (i = 0, j = argStart + 1; i < npL; i++, j++) {
            ...
            if (Tcl_GetDouble(interp, argv[j + 2 * npL], &wt) !=
                TCL_OK) {
                opserr << "WARNING invalid wt\n";
                opserr << "" << argv[1] << " element: " << eleTag <<
                    endln;
        ...
        for (i = 0, j = 1 + (argStart + 1) + 3 * npL; i < npR;
             i++, j++) {
            int sec;
            double pt, wt;
            if (Tcl_GetInt(interp, argv[j], &sec) != TCL_OK) {
            ...
            if (Tcl_GetDouble(interp, argv[j + 2 * npR], &wt) !=
                TCL_OK) {
                opserr << "WARNING invalid wt\n";
                opserr << "" << argv[1] << " element: " << eleTag <<
                    endln;
        ...
        if (argc < 14) {
            ...
            printCommand(argc, argv);
            opserr << "Want: element " << argv[1] <<
                " eleTag? iNode? jNode? transfTag? type distType nIP? secTagI? lpI? secTagJ? lpJ? secTagE?\n";
            return TCL_ERROR;
        }
        ...
        beamIntegr =
            new DistHingeIntegration(lpI, lpJ, *otherBeamInt);
        ...
        if (argc < 16) {
            ...
            printCommand(argc, argv);
            opserr << "Want: element " << argv[1] <<
                " eleTag? iNode? jNode? transfTag? type distType nIP? secTagI? lpI? zetaI? secTagJ? lpJ? zetaJ? secTagE?\n";
            return TCL_ERROR;
        }
        ...
        beamIntegr =
            new RegularizedHingeIntegration(*otherBeamInt, lpI, lpJ,
                                            zetaI, zetaJ);
        ...
        if (argc < 9) {
            ...
            printCommand(argc, argv);
            opserr << "Want: element " << argv[1] <<
                " eleTag? iNode? jNode? transfTag? FixedLocation nIP? secTag1? ... pt1? ... \n";
            return TCL_ERROR;
        }
        ...
        beamIntegr =
            new FixedLocationBeamIntegration(numSections, pts);
    }
        ...
        if (argc < 9) {
            ...
            printCommand(argc, argv);
            opserr << "Want: element " << argv[1] <<
                " eleTag? iNode? jNode? transfTag? LowOrder nIP? secTag1? ... pt1? ... wt1? ...\n";
            return TCL_ERROR;
        }
        ...
        beamIntegr =
            new LowOrderBeamIntegration(numSections, pts, nc, wts);
    }
        ...
        if (argc < 9) {
            ...
            printCommand(argc, argv);
            opserr << "Want: element " << argv[1] <<
                " eleTag? iNode? jNode? transfTag? MidDistance nIP? secTag1? ... pt1? ... \n";
            return TCL_ERROR;
        }
