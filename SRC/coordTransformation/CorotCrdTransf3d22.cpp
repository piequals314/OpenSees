...
// $Source: /usr/local/cvs/OpenSees/SRC/coordTransformation/CorotCrdTransf3d22.cpp,v $
...
// CorotCrdTransf3d22 class. CorotCrdTransf3d22 is a Corot
// transformation for a spatial frame between the global 
// and basic coordinate systems.
//
// Modified by Brighton Laiman from University of California, San Diego (include 
...
#include <CorotCrdTransf3d22.h>
using std::string;
using namespace std;

// initialize static variables
Matrix CorotCrdTransf3d22::RI(3,3); 
Matrix CorotCrdTransf3d22::RJ(3,3); 
Matrix CorotCrdTransf3d22::Rbar(3,3); 
Matrix CorotCrdTransf3d22::e(3,3); 
Matrix CorotCrdTransf3d22::Tp(17,17); 
Matrix CorotCrdTransf3d22::A(3,3);
...
// constructor:
CorotCrdTransf3d22::CorotCrdTransf3d22(int tag, const Vector &vecInLocXZPlane,
                                   const Vector &rigJntOffsetI,
                                   const Vector &rigJntOffsetJ):
CrdTransf(tag, CRDTR_TAG_CorotCrdTransf3d22),
...
nodeIInitialDisp(0), nodeJInitialDisp(0), initialDispChecked(false)
{
    ...
    {
        opserr << "CorotCrdTransf3d22::CorotCrdTransf3d22:  Vector that defines local xz plane is invalid\n";
    ...
    {
        opserr << "CorotCrdTransf3d22::CorotCrdTransf3d22:  Invalid rigid joint offset vector for node I\n";
        opserr << "Size must be 3\n";      
        nodeIOffset.Zero();      
    }
    else
    ...
    {
        opserr << "CorotCrdTransf3d22::CorotCrdTransf3d22:  Invalid rigid joint offset vector for node J\n";
        opserr << "Size must be 3\n";      
        nodeJOffset.Zero(); 
    }
    else
    ...
    {
        opserr << "CorotCrdTransf3d22::CorotCrdTransf3d22: rigid joint zones not implemented yet\n";
    ...
    // Tp is identity(17,17) but swap sign on 2 and 6 (rotations about y)
	    
    // using static matrix (one constant matrix for all objects)
    
    if (Tp(0, 0) == 0)  // initialize only once
    {
        Tp(0, 0)   =  1;
        Tp(1, 1)   =  1;
        Tp(2, 2)   =  -1;
...
CorotCrdTransf3d22::CorotCrdTransf3d22():
CrdTransf(0, CRDTR_TAG_CorotCrdTransf3d22),
vAxis(3), nodeIOffset(3), nodeJOffset(3), xAxis(3),
nodeIPtr(0), nodeJPtr(0), R0(3,3), L(0), Ln(0), 
alphaIq(4), alphaJq(4), 
alphaIqcommit(4), alphaJqcommit(4), alphaI(3), alphaJ(3),
...
int
CorotCrdTransf3d22::commitState(void)
{
    ulcommit      = ul;
...
int
CorotCrdTransf3d22::revertToLastCommit(void)
{
...
int
CorotCrdTransf3d22::revertToStart(void)
{
    ul.Zero();
...
int 
CorotCrdTransf3d22::initialize(Node *nodeIPointer, Node *nodeJPointer)
{       
    int error;
...
int  
CorotCrdTransf3d22::update(void)
{       
    ...
    **************************************************************/
    ...
    // determine global displacement increments from last iteration
    static Vector dispI(11);
    static Vector dispJ(11);

    dispI = nodeIPtr->getTrialDisp();
    dispJ = nodeJPtr->getTrialDisp();

    #ifdef NODEDISP
    // I want to see the displacements
    fprintf(stdout, "CrdTrans Update node i disp\n");
    for (int a = 0; a < 11; ++a)
	fprintf(stdout, "dispI[%2d]: %.2f \n", a, dispI(a));
    fprintf(stdout, "\n");

    fprintf(stdout, "CrdTrans Update node j disp\n");
    for (int a = 0; a < 11; ++a) 
	fprintf(stdout, "dispJ[%2d]: %.2f \n", a, dispJ(a));
    fprintf(stdout, "\n");
    #endif
    ...
    #ifdef DEBUG
    /*
    fprintf(stdout, "CorotCrdTransf3d22::update\n");
    for (int a = 0; a < 4; ++a)
	fprintf(stdout, "gammaq(%1d) = %.2f\n", a+1, gammaq(a));
    fprintf(stdout, "\n");

    for (int a = 0; a < 3; ++a)
	fprintf(stdout, "gammaw(%1d) = %.2f\n", a+1, gammaw(a));
    fprintf(stdout, "\n");

    fprintf(stdout, "dRgamma Matrix:\n");
    for (int a = 0; a < 3; ++a) {
	for (int b = 0; b < 3; ++b)
    	    fprintf(stdout, "%6.2f ", dRgamma(a,b));
	fprintf(stdout, "\n");
    }
    fprintf(stdout, "\n");
    */
    ...
    if (Ln == 0.0) {
	opserr << "\nCorotCrdTransf3d22::update: 0 deformed length\n";
	return -2;  
    }
    ...
    #ifdef ELEDISP
    fprintf(stdout, "Printing out ul (BASIC SYSTEM) in CrdTransf:\n");
    for (int i = 0; i < 17; ++i)
	fprintf(stdout, "ul[%2d] = %.10f\n", i+1, ul(i));
    fprintf(stdout, "\n");
    #endif
...
void
CorotCrdTransf3d22::compTransfMatrixBasicGlobal(void)
{
    ...
    // NOTE: O = 3 x 1 zero vector
    //   T1  = [O'      , (-S(rI3)*e2 + S(rI2)*e3)', O', O', O', O'       , O', 0];
    //   T2  = [(A*rI2)', (-S(rI2)*e1 + S(rI1)*e2)', O', 0, 0  , -(A*rI2)', O', O', 0, 0];
    //   T3  = [(A*rI3)', (-S(rI3)*e1 + S(rI1)*e3)', O', 0, 0  , -(A*rI3)', O', O', 0, 0];
    //   T4  = [O', O', 1 , O', O', O', O', O'];
    //   T5  = [O', O', 0 , 1 , 0 , 0 , O', O', O', O'];
    //   T6  = [O', O', 0 , 0 , 1 , 0 , O', O', O', O'];
    //   T7  = [O', O', O', 1, O' , O', O', O'];
    //   T8  = [O', O', O', 0, 1  , 0 , O', O', O', 0];
    //   T9  = [O'      , O', O', O',  0, 0        , (-S(rJ3)*e2 + S(rJ2)*e3)', O', 0, 0];
    //   T10 = [(A*rJ2)', O', O', 0 ,  0, -(A*rJ2)', (-S(rJ2)*e1 + S(rJ1)*e2)', O', 0, 0];
    //   T11 = [(A*rJ3)', O', O', 0 ,  0, -(A*rJ3)', (-S(rJ3)*e1 + S(rJ1)*e3)', O', 0, 0];
    //   T12 = [O', O', O', O', O', 0 , 0 , 1, O', 0];
    //   T13 = [O', O', O', O', O', O', 1 , O'];
    //   T14 = [O', O', O', O', O', O', 0 , 1, 0, 0];
    //   T15 = [O', O', O', O', O', O', 0 , 0, 1, 0];
    //   T16 = [O', O', O', O', O', O', O', 1];
    ...
    Lr.addMatrixVector(0.0, Lr3, rJ2,  1.0);  //   T(:,8)  += Lr3*rJ2 - Lr2*rJ3;
    Lr.addMatrixVector(1.0, Lr2, rJ3, -1.0);

    for (i = 0; i < 22; i++)
	T(8,i) += Lr(i);
    
    Lr.addMatrixVector(0.0, Lr2, rJ1,  1.0);  //   T(:,9)  += Lr2*rJ1   
    
    for (i = 0; i < 22; i++)
	T(9,i) += Lr(i);
    
    Lr.addMatrixVector(0.0, Lr3, rJ1,  1.0);  //   T(:,10) += Lr3*rJ1
...
// B.L. - This method doesn't seem updated and correct... But since corotational seemed to work
// out properly for our 14 DOF element I'm taking a wild guess it never gets called. 
// To be sure should double check this with breakpoints or some other debugging method.
// UPDATE: Looks like this is a new method they tried to add in but isn't in full effect yet,
// don't need to worry about it for now.
void
CorotCrdTransf3d22::compTransfMatrixBasicGlobalNew(void)
{
    // extract columns of rotation matrices
    int i, j, k;
...
const Vector &
CorotCrdTransf3d22::getBasicIncrDeltaDisp (void)
{
    static Vector dub(17);
    static Vector dul(17);
...
const Vector &
CorotCrdTransf3d22::getBasicIncrDisp(void)
{
    static Vector Dub(17);
    static Vector Dul(17);
...
const Vector &
CorotCrdTransf3d22::getBasicTrialVel(void)
{
    opserr << "ERROR CorotCrdTransf3d22::getBasicTrialVel()"
...
const Vector &
CorotCrdTransf3d22::getBasicTrialAccel(void)
{
    opserr << "ERROR CorotCrdTransf3d22::getBasicTrialAccel()"
        << " - has not been implemented yet." << endln;
...
const Vector &
CorotCrdTransf3d22::getGlobalResistingForce(const Vector &pb, const Vector &unifLoad)
{
    ...
    #ifdef DEBUG
    fprintf(stdout, "CorotCrdTransf3d22::getGlobalResistingForce:\n\n");
    fprintf(stdout, "Basic/Local Forces:\n");
    for (int i = 0; i < 17; ++i)
	fprintf(stdout, "pl[%2d] = %.2f\n", i+1, pl(i));
    fprintf(stdout, "\n\n");

    /*
    fprintf(stdout, "Global Forces:\n");
    for (int i = 0; i < 22; ++i)
	fprintf(stdout, "pg[%2d] = %.2f\n", i+1, pg(i));
    fprintf(stdout, "\n\n");
    */
    #endif
...
const Matrix &
CorotCrdTransf3d22::getGlobalStiffMatrix (const Matrix &kb, const Vector &pb)
{
    ...
    kgConvert.Assemble(A, 0, 0,  pl(16));
    kgConvert.Assemble(A, 0, 6, -pl(16));
    kgConvert.Assemble(A, 6, 0, -pl(16));
    kgConvert.Assemble(A, 6, 6,  pl(16));
    ...
    //ul(0), ul(1), ul(2), ul(8), ul(9), ul(10)
    //pl(0), pl(1), pl(2), pl(8), pl(9), pl(10)
    static Vector ulg(6);
    static Vector plg(6);
    // Tg represents and is equivilent to T matrix found in original
    // CorotCrdTransf3d code.
    ...
    // B.L. - I'm not quite too sure if these next 4 lines of code are entirely right...
    // it seemed to work out well with the 14 DOF element but I feel like it's wrong...
    // From my understanding... Tg is supposed to be equivilent to the original code's
    // T(7,12) matrix. If this is true, then the last row should represent the axial 
    // deformation based of X1/2/3_I and X1/2/3_J.
    /*
    // Modification based off original code
    for (i=0; i < 3; j++) {
	Tg(6, i  ) = T(16,i);
	Tg(6, i+6) = T(16,i+11);
    }
    */
    ...
    // The last row of Tg actualyl doesn't get used for kgConvert or kg...
    // what's the point then of having a 7x12 for T and Tg then...
    // Modified Based of Sydney
    for (j=6; j < 12; j++) {
	Tg(6,j) = T(16,j+5);
    }
    ...
    //       [kgConvert(6,6), 0(6,5), kgConvert(6,6), 0(6,5)
    //        0(5,6)        , 0(5,5), 0(5,6)        , 0(5,5)
    //        kgConvert(6,6), 0(6,5), kgConvert(6,6), 0(6,5)
    //        0(5,6)        , 0(5,5), 0(5,6)        , 0(5,5)
    //
    for (i=0; i<6; i++) {
    ...
    for (i=0; i<6; i++) {
	for (j=11; j<17; j++)
	    kg(i,j) += kgConvert(i,j-5);
    }

    for (i=11; i<17; i++) {
	for (j=0; j<6; j++)
	    kg(i,j) += kgConvert(i-5,j);
    }

    for (i=11; i<17; i++) {
	for (j=11; j<17; j++)
	    kg(i,j) += kgConvert(i-5,j-5);
    }

    #ifdef DEBUG
    fprintf(stdout, "CorotCrdTransf3d22::getGlobalStiffMatrix\n");
    fprintf(stdout, "Printing GLOBAL Stiffness Matrix:\n");
    for (int a = 0; a < 22; ++a) {
	for (int b = 0; b < 22; ++b)
	    fprintf(stdout, "%10.2e ", kg(a,b));
	fprintf(stdout, "\n");
    }
    fprintf(stdout, "\n");
    #endif
...
const Matrix &
CorotCrdTransf3d22::getInitialGlobalStiffMatrix (const Matrix &kb)
{
    // transform tangent stiffness matrix from the basic system to local coordinates
    static Matrix kl(17,17);
...
int 
CorotCrdTransf3d22::getLocalAxes(Vector &XAxis, Vector &YAxis, Vector &ZAxis)
{
    // element projection
    ...
    if (L == 0.0) 
    {
        opserr << "\nCorotCrdTransf3d22::computeElemtLengthAndOrien: 0 length\n";
        return -2;  
    }
    ...
    if (ynorm == 0)
    {
        opserr << "\nCorotCrdTransf3d22::getElementLengthAndOrientation";
...
double 
CorotCrdTransf3d22::getInitialLength(void)
{
    return L;
}
...
double 
CorotCrdTransf3d22::getDeformedLength(void)
{
    return Ln;
}
...
const Vector &
CorotCrdTransf3d22::getQuaternionFromRotMatrix(const Matrix &R) const
{
...
const Vector &
CorotCrdTransf3d22::getQuaternionFromPseudoRotVector(const Vector  &theta) const
{
...
const Vector &
CorotCrdTransf3d22::quaternionProduct(const Vector &q1, const Vector &q2) const
{
...
const Matrix &
CorotCrdTransf3d22::getRotationMatrixFromQuaternion(const Vector &q) const
{ 
    int i, j;
    double factor;
...
const Vector &
CorotCrdTransf3d22::getTangScaledPseudoVectorFromQuaternion(const Vector &q) const
{ 
    static Vector w(3);
...
const Matrix &
CorotCrdTransf3d22::getRotMatrixFromTangScaledPseudoVector(const Vector &w) const
{ 
...
const Matrix &
CorotCrdTransf3d22::getSkewSymMatrix (const Vector &theta) const
{
    static Matrix S(3,3);
...
const Matrix &
CorotCrdTransf3d22::getLMatrix (const Vector &ri) const
{
    ...
    L.Assemble(L1, 11, 0, -1.0);
    L.Assemble(L2, 14, 0,  1.0);
...
const Matrix &
CorotCrdTransf3d22::getKs2Matrix (const Vector &ri, const Vector &z) const
{
...
CrdTransf *
CorotCrdTransf3d22::getCopy3d(void)
{
    // create a new instance of CorotCrdTransf3d22 
    
    CorotCrdTransf3d22 *theCopy = new CorotCrdTransf3d22 (this->getTag(), vAxis, nodeIOffset, nodeJOffset);
    
    if (!theCopy)
    {
        opserr << "CorotCrdTransf3d22::getCopy() - out of memory creating copy\n";
        exit(-1);
        return 0;
    }    
...
int 
CorotCrdTransf3d22::sendSelf(int cTag, Channel &theChannel)
{
    Vector data(57);
    for (int i=0; i<12; i++) 
        data(i) = ulcommit(i);
    for (int j=0; j<4; j++) {
        data(12+j) = alphaIqcommit(j);
        data(15+j) = alphaJqcommit(j);
    }
    
    for (int k=0; k<3; k++) {
        data(18+k) = xAxis(k);
        data(21+k) = vAxis(k);
        data(24+k) = nodeIOffset(k);
        data(27+k) = nodeJOffset(k);
        data(30+k) = alphaI(k);
        data(33+k) = alphaJ(k);
    }
    
    if (nodeIInitialDisp != 0) {
        data(36) = nodeIInitialDisp[0];
        data(37) = nodeIInitialDisp[1];
        data(38) = nodeIInitialDisp[2];
        data(39) = nodeIInitialDisp[3];
        data(40) = nodeIInitialDisp[4];
        data(41) = nodeIInitialDisp[5];
        data(42) = nodeIInitialDisp[6];
        data(43) = nodeIInitialDisp[7];
        data(44) = nodeIInitialDisp[8];
        data(45) = nodeIInitialDisp[9];
        data(46) = nodeIInitialDisp[10];
    } else {
        data(36) = 0.0;
        data(37) = 0.0;
        data(38) = 0.0;
    ...
    if (nodeJInitialDisp != 0) {
        data(47) = nodeJInitialDisp[0];
        data(48) = nodeJInitialDisp[1];
        data(49) = nodeJInitialDisp[2];
        data(50) = nodeJInitialDisp[3];
        data(51) = nodeJInitialDisp[4];
        data(52) = nodeJInitialDisp[5];
        data(53) = nodeJInitialDisp[6];
        data(54) = nodeJInitialDisp[7];
        data(55) = nodeJInitialDisp[8];
        data(56) = nodeJInitialDisp[9];
        data(57) = nodeJInitialDisp[10];
    } else {
        data(47) = 0.0;
        data(48) = 0.0;
        data(49) = 0.0;
...
int 
CorotCrdTransf3d22::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
    Vector data(57);
    ...
    for (i=0; i<12; i++) 
        ulcommit(i) = data(i);
    for (j=0; j<4; j++) {
        alphaIqcommit(j) = data(12+j);
        alphaJqcommit(j) = data(15+j);
    }
    
    for (int k=0; k<3; k++) {
        xAxis(k)       = data(18+k);
        vAxis(k)       = data(21+k); 
        nodeIOffset(k) = data(24+k);
        nodeJOffset(k) = data(27+k);
        alphaI(k)      = data(30+k);
        alphaJ(k)      = data(33+k);
    }
...
const Vector &
CorotCrdTransf3d22::getPointGlobalCoordFromLocal(const Vector &xl)
{
    static Vector xg(3);
    opserr << " CorotCrdTransf3d22::getPointGlobalCoordFromLocal: not implemented yet" ;
...
const Vector &
CorotCrdTransf3d22::getPointGlobalDisplFromBasic (double xi, const Vector &uxb)
{
    static Vector uxg(3);
    opserr << " CorotCrdTransf3d22::getPointGlobalDisplFromBasic: not implemented yet" ;
...
{
    s << "\nCrdTransf: " << this->getTag() << " Type: CorotCrdTransf3d22";
    s << "\tvAxis: " << vAxis;
