...
// $Source: /usr/local/cvs/OpenSees/SRC/coordTransformation/LinearCrdTransf3d22.cpp,v $
...
// LinearCrdTransf3d22 class. LinearCrdTransf3d22 is a linear
...
CrdTransf(0, CRDTR_TAG_LinearCrdTransf3d22),
nodeIPtr(0), nodeJPtr(0),
L(0), nodeIInitialDisp(0), nodeJInitialDisp(0), initialDispChecked(false)
{
    for (int i = 0; i < 3; i++)
...
int
LinearCrdTransf3d22::commitState(void)
{
    return 0;
}
...
int
LinearCrdTransf3d22::revertToLastCommit(void)
{
    return 0;
}
...
int
LinearCrdTransf3d22::revertToStart(void)
{
    return 0;
}
...
int 
LinearCrdTransf3d22::initialize(Node *nodeIPointer, Node *nodeJPointer)
{       
    int error;
    ...
    {
        opserr << "\nLinearCrdTransf3d22::initialize";
...
int
LinearCrdTransf3d22::update(void)
{
    return 0;
}
...
int 
LinearCrdTransf3d22::computeElemtLengthAndOrient()
{
    // element projection
    static Vector dx(3);
    ...
    if (L == 0.0) {
        opserr << "\nLinearCrdTransf3d22::computeElemtLengthAndOrien: 0 length\n";
        return -2;  
    }
...
LinearCrdTransf3d22::getLocalAxes(Vector &XAxis, Vector &YAxis, Vector &ZAxis)
{
    ...
    if (ynorm == 0) {
        opserr << "\nLinearCrdTransf3d22::getLocalAxes";
...
double 
LinearCrdTransf3d22::getInitialLength(void)
{
    return L;
}
...
double 
LinearCrdTransf3d22::getDeformedLength(void)
{
    return L;
}
...
const Vector &
LinearCrdTransf3d22::getBasicTrialDisp (void)
{
    ...
    static double ul[22];
    // these need to be changed for 22 dofs, keep 7-11 and 18-22 the same
    ul[0]  = R[0][0]*ug[0] + R[0][1]*ug[1] + R[0][2]*ug[2];
    ul[1]  = R[1][0]*ug[0] + R[1][1]*ug[1] + R[1][2]*ug[2];
    ul[2]  = R[2][0]*ug[0] + R[2][1]*ug[1] + R[2][2]*ug[2];
...
const Vector &
LinearCrdTransf3d22::getBasicIncrDeltaDisp(void)
{
    // determine global displacements
...
const Vector &
LinearCrdTransf3d22::getBasicTrialVel(void)
{
...
const Vector &
LinearCrdTransf3d22::getBasicTrialAccel(void)
{
...
const Vector &
LinearCrdTransf3d22::getGlobalResistingForce(const Vector &pb, const Vector &p0)
{
...
const Matrix &
LinearCrdTransf3d22::getGlobalStiffMatrix (const Matrix &KB, const Vector &pb)
{
    ...
    /*
    opserr << "Basic Stiffness Matrix" << endln;
    for (i = 0; i < 17; i++) {
	for (j = 0; j < 17; j++)
	    fprintf(stdout, "%12.2e ", kb[i][j]);
	fprintf(stdout, "\n");
    }
    fprintf(stdout, "\n");
    */
    ...
    /*
    opserr << "Tangent Local Stiffness Matrix" << endln;
    for (i = 0; i < 22; i++) {
	for (j = 0; j < 22; j++)
	    fprintf(stdout, "%12.2e ", kl[i][j]);
	fprintf(stdout, "\n");
    }
    fprintf(stdout, "\n");
    */
    ...
    /*
    opserr << "Tangent Global Stiffness Matrix" << endln;
    for (i = 0; i < 22; i++) {
	for (j = 0; j < 22; j++)
	    fprintf(stdout, "%12.2e ", kg(i,j));
	fprintf(stdout, "\n");
    }
    fprintf(stdout, "\n");
    */
...
const Matrix &
LinearCrdTransf3d22::getInitialGlobalStiffMatrix (const Matrix &KB)
{
    static Matrix kg(22,22);	// Global stiffness for return
    static double kb[17][17];	// Basic stiffness
    static double kl[22][22];	// Local stiffness
	...
	/*
	opserr << "Initial Local Stiffness Matrix" << endln;
	for (i = 0; i < 22; i++) {
	    for (j = 0; j < 22; j++)
		fprintf(stdout, "%12.2f ", kl[i][j]);
	    fprintf(stdout, "\n");
	}
	fprintf(stdout, "\n");
	*/
...
CrdTransf *
LinearCrdTransf3d22::getCopy3d(void)
{
    // create a new instance of LinearCrdTransf3d22 
    
    LinearCrdTransf3d22 *theCopy;
...
int 
LinearCrdTransf3d22::sendSelf(int cTag, Channel &theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr << "LinearCrdTransf3d22::sendSelf - failed to send Vector\n";
...
int 
LinearCrdTransf3d22::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr << "LinearCrdTransf3d22::recvSelf - failed to receive Vector\n";
...
const Vector &
LinearCrdTransf3d22::getPointGlobalCoordFromLocal(const Vector &xl)
{
    static Vector xg(3);
...
const Vector &
LinearCrdTransf3d22::getPointGlobalDisplFromBasic (double xi, const Vector &uxb)
{
    // determine global displacements
    const Vector &disp1 = nodeIPtr->getTrialDisp();
    const Vector &disp2 = nodeJPtr->getTrialDisp();
